...
describe "[Start a Game]", :type => :request do

=begin
    Sample Request /v1/deck/start?gameid={code}&userid={userid}
    Sample Response
            {
...
=end
  ...
  it "can not submit without paramter", :focus => true do
    get "/v1/deck/start?userid=user-fdasnjf"
    expect(response).to have_http_status(:bad_request)
  ...
  it "can not submit without paramter", :focus => true do
    get "/v1/deck/start?gameid=daSFnjf"
    expect(response).to have_http_status(:bad_request)
  ...
  it "valid response with deck data", :focus => true do
    ...
    get "/v1/deck/start?gameid=daSFnjf&userid=user-fdasnjf"
    ...
    close_result = close_data["gamedata"]
